...
const Profile = () => {
  ...
  const { data: filteredUser } = useSingleUserQuery(userId);
  const {
  ...
  const filteredUserAccount = userAccountsData?.find(
    (SingleUserAccount: IUser) => {
      return SingleUserAccount?.user === filteredUser?.username;
    }
  );
  // const {
  //   data: userData,
  ...
  const handlePayment = (orderId: number, orderCost: number) => {
    ...
    const updatedAccount = {
      id: id,
      data: {
        amount: filteredUserAccount?.amount - orderCost,
      },
    };
  ...
  const categorise1 = () => {
    ...
    } else if (!userLoading && filteredUser?.length === 0) {
      return (
        <div className="my-[100px] flex flex-col justify-center items-center">
          <img
            src="https://ph-tube.netlify.app/images/Icon.png"
